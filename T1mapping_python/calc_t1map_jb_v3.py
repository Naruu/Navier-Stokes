...
# yck, 2019-11-07
# matlab to python. hll, 2019-11-09
...
def getNLSStruct_v2(*args,**kwargs):
	...
	print(tVec_t.shape)
	print(nlsS['theExp'].shape)
	print(nlsS['rhoNormVec'].shape)
...
if __name__ == '__main__':
	...
	fname1 = 'T1map_pre_post20min_3141____.pkl'
	data = pickle.load(open(fname1, 'rb'))
	print(data.keys())

	t1map_pre = data['t1map_pre']
	t1map_post = data['t1map_post']
	IRimg_pre = data['IRimg_pre']
	IRimg_post = data['IRimg_post']
	# Invtime_pre = data['Invtime_pre']
	Invtime_post = data['Invtime_post']
	...
	print(extra)
	extra.tVec = Invtime_post
	extra.N = Invtime_post.shape[0]
	...
	print(nlsS.keys())
	# nlsS['tVec'] = Invtime_post
	# nlsS['N'] = Invtime_post.shape[0]
	...
	for jj in range(nVoxAll):
		if jj % 10000 == 0:
			print('jj=%d out of %d' % (jj, nVoxAll))
	...
	plt.figure()
	plt.subplot(121)
	plt.imshow(t1map_post, cmap='jet', clim=[0, 600])
	plt.subplot(122)
	plt.imshow(T1[:,:,0,0], cmap='jet', clim=[0, 600])
	plt.title('T1 map by J Barral')
	plt.show()
